...
class Scene(Container):
    ...
    * leave_progress_bars\n
    ...
    def construct(self):
        """
        The function which encapsulates all your animations.
        Manim calls this function to render your Scene. 
        """
    ...
    def get_indices(self, mob,size=0.19):
        for index in range(len(mob)):
            direction = UP if index == 0 or index % 2 == 0 else DOWN

            self.play(Write(Text(f"{index}", size=size, opacity=0.3,stroke_width=0.3,color=RED)
                            .next_to(mob[index], direction)))
    
    def play_many(self,anim,*mobjects,**kwargs):
        for mob in mobjects:
            self.play(anim(mob),**kwargs)
